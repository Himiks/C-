...
int main() {
	...
	do {
		...
		std::cin >> symbolFrom;
		transform(symbolFrom.begin(), symbolFrom.end(), symbolFrom.begin(), ::toupper);
	...
	do {
		std::cout << "Please enter a currency to convert to: ";
		std::cin >> symbolTo;
		transform(symbolTo.begin(), symbolTo.end(), symbolTo.begin(), ::toupper); 
...
void loadCurrencyData(std::string filePath) {
	std::cout << "Opening cvs file..." << std::endl;
	std::ifstream in;
	in.open(filePath);
